    ...
    fn create_lane_test() {
        ...
        for test_case in test_cases {
            ...
            if test_case.sig.is_some() && test_case.secret_preimage.is_empty() {
                ...
                expect_authenticate_message(&mut rt, payer_addr, sv.clone());
            }
...
mod merge_tests {
    ...
    fn failure_end(rt: &mut MockRuntime, sv: SignedVoucher, exp_exit_code: ExitCode) {
        let payee_addr = Address::new_id(PAYEE_ID);
        expect_authenticate_message(rt, payee_addr, sv.clone());
    ...
    fn merge_success() {
        ...
        let payee_addr = Address::new_id(PAYEE_ID);
        expect_authenticate_message(&mut rt, payee_addr, sv.clone());
...
mod update_channel_state_extra {
    ...
    fn construct_runtime(exit_code: ExitCode) -> (MockRuntime, SignedVoucher) {
        ...
        expect_authenticate_message(&mut rt, state.to, sv.clone());
...
fn expect_authenticate_message(rt: &mut MockRuntime, payer_addr: Address, sv: SignedVoucher) {
    rt.expect_send(
        payer_addr,
